...
//定义一个函数：两个数组进行对比，对比出当前用户到底显示哪些异步路由
 const computedAsyncRoutes = (asyncRoutes,routes)=>{
    ...
    return asyncRoutes.filter(item=>{//数组当中没有这个元素返回索引值-1，如果有这个元素返回的索引值一定不是-1 
         
        if(routes.indexOf(item.name)!=-1){//判断服务器返回的数据中，有包含异步路由的名字
        //这样只能过滤一级路由
        // 所以我们还要用到递归，过滤二三四...级路由
        //递归(递归的目的是过滤二三四...级路由)
          if(item.children&&item.children.length){//item是一级路由
              item.children = computedAsyncRoutes(item.children,routes);
              //如果item有children并且children有长度(也就是有children里面有值)
          }
          return true;
        }
     })
...
const actions = {
  ...
  async login({ commit }, userInfo) {
    ...
    const { username, password } = userInfo;
      // 他在这里没有用到async和await 我们把他改一改 改为async和await
  ...
  getInfo({ commit, state }) {
    ...
    return new Promise((resolve, reject) => {
      getInfo(state.token).then(response => {
        ...
        //计算出的异步路由 并且把异步路由和返回的路由做对比的computedAsyncRoutes函数作为参数
